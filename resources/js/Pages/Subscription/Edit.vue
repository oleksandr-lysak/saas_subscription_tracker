<template>
  <div class="max-w-xl mx-auto py-10 px-4">
    <div class="bg-white rounded-lg shadow-lg p-8">
      <h1 class="text-2xl font-extrabold mb-6 text-gray-800">Edit Subscription</h1>
      <form @submit.prevent="submit" class="space-y-5">
        ...
        <div class="flex justify-end gap-2 mt-6">
          <button class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold px-6 py-2 rounded-md shadow transition" type="reset" @click="form.reset()">Reset</button>
          <button class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2 rounded-md shadow transition" type="submit">Update</button>
        </div>
      </form>
    </div>
  </div>
...
const subscription = props.subscription.data ?? props.subscription;
const form = useForm({
...
function submit() {
  form.put(route('subscriptions.update', subscription.id));
}
</script> 